    ...
    expenseList.innerHTML += `
            <tr class="border-b border-slate-600 align-middle">
                ...
                <td class="p-3 text-center break-words">
                    <button onclick="editExpense(${originalIndex})" class="bg-blue-700 px-1 py-1 rounded">
                        ✏️
                    </button>
                    <button onclick="deleteExpense(${originalIndex})" class="bg-red-700 px-1 py-1 rounded">
                        🗑️
                    </button>
                </td>
            </tr>
...
function applySorting(expenseArray) {
  ...
  expenseArray.sort((a, b) =>
    sortState.direction === "desc" ? compare(b, a) : compare(a, b),
  );
  return expenseArray;
}
...
// TODO: add a categoryFilter function